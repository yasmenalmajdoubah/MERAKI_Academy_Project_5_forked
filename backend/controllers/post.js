...
const getPostsMyFollows = (req, res) => {
  ...
  const query = `SELECT posts.body, posts.image,posts.post_id, posts.created_at, users.firstname , users.lastname, users.profileimage FROM follows INNER JOIN posts ON user_id=follows.followed_user_id INNER JOIN users ON posts.user_id=users.user_id WHERE following_user_id=$1
 `;
 /*
UNION
